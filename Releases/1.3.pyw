...
#-------<Install tools>-----
    ...
    def Uninstalling_DNS():
        powershell(command="Uninstall-WindowsFeature -Name 'DNS' –IncludeManagementTools",finish=True)
    ...
    def Uninstalling_iSCSI():
        powershell(command="Uninstall-WindowsFeature -Name FS-iSCSITarget-Server",finish=True)
    ...
    def installing_iSCSI():
        powershell(command="Install-WindowsFeature -Name FS-iSCSITarget-Server",finish=True)
...
def setup_DHCP_interface():
    ...
    StartRangelabel.grid(row=1, column=0)
    StartRangeentry = tk.Entry(root)
    StartRangeentry.grid(row=1, column=1)
    EndRangelabel = tk.Label(root, text='EndRange:')
    EndRangelabel.grid(row=2, column=0)
    EndRangeentry = tk.Entry(root)
    EndRangeentry.grid(row=2, column=1)
    SubnetMasklabel = tk.Label(root, text='SubnetMask:')
    SubnetMasklabel.grid(row=3, column=0)
    SubnetMaskentry = tk.Entry(root)
    SubnetMaskentry.grid(row=3, column=1)
    ScopeNamelabel = tk.Label(root, text='ScopeName:')
    ScopeNamelabel.grid(row=4, column=0)
    ScopeNameentry = tk.Entry(root)
    ScopeNameentry.grid(row=4, column=1)
    DNS_Addresslabel = tk.Label(root, text='DNS Address:')
    DNS_Addresslabel.grid(row=5, column=0)
    DNS_Addressentry = tk.Entry(root)
    DNS_Addressentry.grid(row=5, column=1)
    Routerlabel = tk.Label(root, text='Router IP:')
    Routerlabel.grid(row=6, column=0)
    Routerentry = tk.Entry(root)
    Routerentry.grid(row=6, column=1)
...
def Remove_DHCP_Scope_interface():
    ...
    ScopeIDlabel.grid(row=1, column=0)
    ScopeIDentry = tk.Entry(root)
    ScopeIDentry.grid(row=1, column=1)
    Back=ttk.Button(root, text="Back", command=back, style='Red.TButton')
    Back.grid(row=2,column=0)
    read_input=ttk.Button(root, text="Finish", command=DHCP_Finish_Button, style='Green.TButton')
    read_input.grid(row=2,column=1) 
def setup_DNS_interface():
    if "Foward_Lookup_Zone" in current_interface:
        if "Add_DNS_Primary_Zone" in current_interface:
            ...
            def DNS_Record_input_Click():
                ...
                if tmp=="A":
                    ...
                    IPv4Address_Label.grid(row=2,column=0)
                    IPv4Address_Entry=tk.Entry(root)
                    IPv4Address_Entry.grid(row=2,column=1)
            ...
            DNS_Record_input.grid(row=3,column=1)
            ...
            RecordNamelabel.grid(row=2, column=0)
            RecordNameentry = tk.Entry(root)
            RecordNameentry.grid(row=2, column=1)
            Set_Record_Type_label=tk.Label(root,text="Type")
            Set_Record_Type_label.grid(row=3,column=0)
            Set_Record_Type=ttk.Combobox(root,values=["A","AAAA","CName"])
            Set_Record_Type.grid(row=3,column=1)
            Back=ttk.Button(root, text="Back", command=back, style='Red.TButton')
            Back.grid(row=4,column=0)
            read_input=ttk.Button(root, text="Finish", command=DNS_Finish_Button, style='Green.TButton')
            read_input.grid(row=4,column=1)    
        else:
            Windowstatus = tk.Label(root, text=windowstatusconfig, bd=1, relief=tk.SUNKEN, anchor=tk.CENTER)
            Windowstatus.pack(side=tk.TOP, fill=tk.X)
        ...
        if "Add_DNS_Reverse_Zone" in current_interface:
            ...
            NetworkID_Label.grid(row=1,column=0)
            NetworkID_Entry=tk.Entry(root)
            NetworkID_Entry.grid(row=1,column=1)
            ...
            Add_Reverse_Zone_input.grid(row=2,column=1)
            ...
            ZoneName_ID_Label.grid(row=2,column=0)
            ZoneName_Entry=tk.Entry(root)
            ZoneName_Entry.grid(row=2,column=1)
            PTRDomainName_Label=tk.Label(root,text="PTRDomainName:")
            PTRDomainName_Label.grid(row=3,column=0)
            PTRDomainName_Entry=tk.Entry(root)
            PTRDomainName_Entry.grid(row=3,column=1)
            Exit=ttk.Button(root,text="Exit",command=back, style='Red.TButton')
            ...
            Add_PTR_Record_input.grid(row=4,column=1)
            ...
            ZoneName_Label.grid(row=2,column=0)
            ZoneName_Entry=tk.Entry(root)
            ZoneName_Entry.grid(row=2,column=1)
            Exit=ttk.Button(root,text="Exit",command=back, style='Red.TButton')
        ...
        Fowarder_input.grid(row=2,column=1)
    else:
        Windowstatus = tk.Label(root, text=windowstatusconfig, bd=1, relief=tk.SUNKEN, anchor=tk.CENTER)
        Windowstatus.pack(side=tk.TOP, fill=tk.X)
...
def Setup_iSCSI_Disk_Share_interface():
    ...
    if "Add_VirtualDisk" in current_interface:
        ...
        VDisk_NameEntry.grid(row=2,column=1)
        VDisk_SizeLabel=tk.Label(root,text="VDisk Size(Ex:10GB)")
        VDisk_SizeLabel.grid(row=3,column=0)
        VDisk_SizeEntry=tk.Entry(root)
        VDisk_SizeEntry.grid(row=3,column=1)
        Exit=ttk.Button(root,text="Exit",command=back, style='Red.TButton')
        Exit.grid(row=4,column=0)
        ...
        TargetnameLabel.grid(row=1,column=0)
        TargetnameEntry=tk.Entry(root)
        TargetnameEntry.grid(row=1,column=1)
        ...
        Share_VDisk_input.grid(row=5,column=1)
    else:
        Windowstatus = tk.Label(root, text=windowstatusconfig, bd=1, relief=tk.SUNKEN, anchor=tk.CENTER)
        Windowstatus.pack(side=tk.TOP, fill=tk.X)
